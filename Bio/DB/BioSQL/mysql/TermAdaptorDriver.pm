...
sub store_synonym{
    ...
    my $lucachekey = "LOOKUP SYNONYM";
    my $isth = $adp->sth($icachekey);
    my $lusth = $adp->sth($lucachekey);
	...
	$isth = $self->_build_sth($adp, $icachekey,
				  ...
				  ") VALUES (?, ?)");
	$lusth = $self->_build_sth($adp, $lucachekey,
				   "SELECT ".
				   join(", ", $colmap->{'synonym'}, $fkname).
				   " FROM $table WHERE ".
				   join(" AND ",
	...
	$adp->debug("$lucachekey: $dbgmsg\n");
	if(! $lusth->execute($syn, $obj->primary_key())) {
	    $self->throw("failed to execute $lucachekey: ".$lusth->errstr);
	}
	# set to TRUE but zero rows inserted if lookup is successful
	$rv = "0E0" if $lusth->fetchrow_arrayref();
    }
    return $rv;
}
