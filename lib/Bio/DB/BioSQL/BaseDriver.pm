...
sub insert_object {
    ...
    if ( $rv && ( $rv != 0 ) ) {
        ...
        $pk = $adp->dbcontext()->dbi()
            ->last_id_value( $dbh, $self->sequence_name($table) );
    } elsif ( ! $rv ) {    # note this is *not* equivalent to $rv == 0 !
                           # the statement failed
        $self->report_execute_failure(
            -sth     => $sth,
            -adaptor => $adp,
            -op      => 'insert',
            -vals    => $slotvals,
...
sub update_object {
    ...
    my $sth
        = $self->get_sth( $adp, $obj, $fkobjs, $cache_key, 'update_object' );
    ...
    while ( $i < @slots ) {
        if ( $slotmap->{ $slots[$i] }
            ...
            if ( $adp->verbose > 0 ) {
                $adp->debug(
                    sprintf(
                        "%s::update: binding column %d to \"%s\"(%s)\n",
                        substr( ref($adp), rindex( ref($adp), "::" ) + 2 ),
                        $j,
                        $slotvals->[$i] || '',
                        ( $slots[$i] )
    ...
    if ( ! $tbl ) {
        ...
        if ( ref($obj) ) {
            ...
            if ( ! ( $tbl || $obj->isa("Bio::DB::PersistenceAdaptorI") ) ) {
                ...
                if (@classes) {
                    @classes
                        = &_order_classes_by_inheritance( $obj, @classes );
...
sub association_table_name {
    ...
    if ( ref($assocmap) ) {
        $self->throw(
            "association table for classes (" . join(
                ",",
                map {
                    ref($_)
                        ? ( $_->isa("Bio::DB::PersistentObjectI")
...
=head2 objrel_map
 ...
 Title   : objrel_map
 Usage   :
 Function: Get/set the object-relational map from .
 Example :
...
sub report_execute_failure {
    ...
    my $msg = "$op in "
        . ref($adp)
        . " (driver) failed, values were (\""
        . join( "\",\"", @$slotvals ) . "\")";
    if ($fkobjs) {
        $msg .= " FKs ("
            . join( ",",
            map { $_ && ref($_) ? $_->primary_key() : "<NULL>"; } @$fkobjs )
            . ")";
    }
    $self->$reportfunc( "$msg\n" . $sth->errstr );
}
